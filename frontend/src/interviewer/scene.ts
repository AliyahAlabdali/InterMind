...
import type { InterviewerPose } from "./types"
...
export interface InterviewerSceneOptions {
  /** Drop continuous animation and move between poses in single short steps instead. */
  reducedMotion?: boolean
  tone?: InterviewerTone
}

export interface InterviewerSceneHandle {
  setPose: (pose: InterviewerPose) => void
  setPointer: (x: number, y: number) => void
  setLevel: (level: number) => void
  /** One-shot resolve sweep, for the moment evidence is recorded. */
  pulse: () => void
  /** Pause/resume the render loop, e.g. when scrolled out of view. */
...
/** A representative amplitude used instead of live audio when motion is reduced, so that
 *  "speaking" still has its own shape without anything tracking the voice in real time. */
const REDUCED_SPEAKING_LEVEL = 0.4
 ...
 */
export function createInterviewerScene(
  canvas: HTMLCanvasElement,
  options: InterviewerSceneOptions = {},
): InterviewerSceneHandle | null {
  ...
  const palette = TONE_PALETTE[options.tone ?? "onLight"]
  ...
  function applyOrientation(elapsed: number, drift: boolean) {
    armature.rotation.x = -0.42 + pointerEased.y * 0.24
    armature.rotation.y = (drift ? elapsed * 0.05 : 0.35) + pointerEased.x * 0.5
    armature.rotation.z = 0.16 + pointerEased.x * 0.06
  }

  function render(now: number) {
    const elapsed = (now - startedAt) / 1000

    levelEased = lerp(levelEased, level, 0.2)
    pointerEased.x = lerp(pointerEased.x, pointer.x, 0.05)
    pointerEased.y = lerp(pointerEased.y, pointer.y, 0.05)

    if (sweepEnergy > 0.001) {
      sweep = (sweep + 0.014) % 1
      sweepEnergy *= 0.955
    } else {
      sweepEnergy = 0
    ...
    shared.uSweep.value = sweep
    shared.uSweepEnergy.value = sweepEnergy
    // A small idle wave keeps the instrument alive even with no speech running through it.
    shared.uWave.value = lerp(shared.uWave.value, 0.012 + levelEased * 0.075, 0.15)

    shared.uOpen.value = lerp(shared.uOpen.value, target.open, 0.055)
    shared.uTilt.value = lerp(shared.uTilt.value, target.tilt, 0.055)
    shared.uSpin.value = lerp(shared.uSpin.value, target.spin, 0.04)
    shared.uScatter.value = lerp(shared.uScatter.value, target.scatter, 0.05)
    shared.uOrder.value = lerp(shared.uOrder.value, target.order, 0.05)
    shared.uCharge.value = lerp(shared.uCharge.value, target.charge, 0.07)
    shared.uFocus.value = lerp(shared.uFocus.value, target.focus, 0.05)

    applyOrientation(elapsed, true)
    renderer.render(scene, camera)
    if (running) frame = requestAnimationFrame(render)
  }

  function startLoop() {
    if (reduced || running || disposed || !active || !visible) return
    running = true
  ...
  /**
   * Reduced motion: one short eased step into the new configuration, then completely still.
   * The presence stays, and because every state is its own shape the change is fully legible
   * without anything looping.
   */
  function stepToPose(durationMs = 460) {
    if (disposed) return
    cancelAnimationFrame(stepFrame)
    const from = {
      open: shared.uOpen.value,
      tilt: shared.uTilt.value,
      spin: shared.uSpin.value,
      scatter: shared.uScatter.value,
      order: shared.uOrder.value,
      charge: shared.uCharge.value,
      focus: shared.uFocus.value,
      wave: shared.uWave.value,
    }
    const toWave = 0.012 + level * 0.075
    const begun = performance.now()

    const tick = (now: number) => {
      const raw = Math.min((now - begun) / durationMs, 1)
      const e = 1 - Math.pow(1 - raw, 3)
      shared.uOpen.value = from.open + (target.open - from.open) * e
      shared.uTilt.value = from.tilt + (target.tilt - from.tilt) * e
      shared.uSpin.value = from.spin + (target.spin - from.spin) * e
      shared.uScatter.value = from.scatter + (target.scatter - from.scatter) * e
      shared.uOrder.value = from.order + (target.order - from.order) * e
      shared.uCharge.value = from.charge + (target.charge - from.charge) * e
      shared.uFocus.value = from.focus + (target.focus - from.focus) * e
      shared.uWave.value = from.wave + (toWave - from.wave) * e
      // A fixed sample of the clock, so the still frame is a composed pose rather than
      // whatever phase the animation happened to stop at.
      shared.uTime.value = 1.4
      applyOrientation(0, false)
      renderer.render(scene, camera)
      if (raw < 1) stepFrame = requestAnimationFrame(tick)
    }
    stepFrame = requestAnimationFrame(tick)
  }
  ...
  // A lost context used to leave a permanently blank canvas, which on a phone mid-interview
  // meant the interviewer simply vanished. Preventing the default lets the browser hand the
  // context back, and the restore path redraws.
  ...
  return {
    setPose(pose) {
      target.open = pose.open
      target.tilt = pose.tilt
      target.spin = pose.spin
      target.scatter = pose.scatter
      target.order = pose.order
      target.charge = pose.charge
      target.focus = pose.focus
      if (reduced) stepToPose()
    },
    setPointer(x, y) {
      if (reduced) return
      pointer.x = x
    ...
    setLevel(next) {
      if (reduced) {
        // Held, not tracked: a static stand-in amplitude keeps the speaking shape distinct
        // without anything following the voice frame by frame.
        const held = next > 0 ? REDUCED_SPEAKING_LEVEL : 0
        if (held !== level) {
          level = held
          stepToPose(340)
        }
    ...
    pulse() {
      ...
      sweepEnergy = 1
      if (!reduced) return
      // Reduced motion keeps the evidence moment as one short pass rather than a decaying
      // flare. Dropping it entirely would lose a real product event.
      const begun = performance.now()
      const tick = (now: number) => {
        const raw = Math.min((now - begun) / 640, 1)
        shared.uSweep.value = raw
        shared.uSweepEnergy.value = 1 - raw
        shared.uTime.value = 1.4
        renderer.render(scene, camera)
